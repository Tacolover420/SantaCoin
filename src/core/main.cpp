...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(bDevOpsPayment)
    {
        ...
        // Define primitives depending if PoW
        if (!isProofOfStake) {
                   ...
                   if (addressOut.ToString() == strVfyDevopsAddress) {
                      LogPrintf("CheckBlock() : PoW Recipient devops address validity succesfully verified\n");
                       ...
                       if (pindexBest->nHeight > 2) {
                            ...
                            LogPrintf("CheckBlock() : PoW Recipient devops address skipping during chain initial startup\n");
                       }
                   }
        ...
        if (fBlockHasPayments) {
            LogPrintf("CheckBlock() : PoW non-miner reward payments succesfully verified\n");
        } else {
            LogPrintf("CheckBlock() : PoW non-miner reward payments could not be verified\n");
            return DoS(100, error("CheckBlock() : PoW invalid payments in current block\n"));
        }
    }
