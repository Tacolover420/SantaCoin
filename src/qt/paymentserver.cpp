...
const int SANTACOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString SANTACOIN_IPC_PREFIX("SantaCoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(SANTACOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SANTACOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(SANTACOIN_IPC_CONNECT_TIMEOUT);
