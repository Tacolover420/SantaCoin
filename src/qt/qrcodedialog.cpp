...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // start the display unit, uses the default ("SANTA")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SANTA unit input in lnReqAmount, we generate the URI with SANTA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SANTA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
